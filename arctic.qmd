...
#| message: false
  ...
  chartjs(palette = customColors) %>%
    ...
    cjsSeries(data = baseline_df$lower_extent ,  borderDash=c(5,5), label = sprintf("Lower (Stdev) : (1985-%4d)", lastyear)) %>%
    cjsSeries(data = baseline_df$upper_extent , borderDash = c(5,5), label = sprintf("Upper (Stdev): (1985-%4d)", lastyear))
